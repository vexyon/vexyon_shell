...
# old .conf files instead of leaving them to rot next to the Lua ones. Only our
# own files are touched (they all carry the Vexyon marker).
retire_conf() {
...
#   theme.json     snapshot estático del tema (re-sync: vexyon-greeter-sync-theme)
...
if [ "${VEXYON_GREETER:-1}" != "0" ]; then
  ...
  install_greeter() {
    ...
    rm -f "$kconf"
    sudo install -Dm644 "$greeter_theme" /etc/greetd/vexyon-greeter/theme.json
