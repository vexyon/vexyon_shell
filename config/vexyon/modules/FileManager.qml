...
FloatingWindow {
    ...
    Component.onCompleted: { refresh(); refreshVolumes(); fmKeys.forceActiveFocus(); }
    onVisibleChanged: {
        ...
        ColumnLayout {
            ...
            Item {
                ...
                MouseArea {
                    ...
                    // de fichero. startDrag bloquea en un event loop anidado hasta el
                    // drop; `cancelled = true` deja el resto del press interno mudo.
                    property bool externalStarting: false
                    function startExternalDrag() {
                        if (externalStarting) return;
                        externalStarting = true;
                        cancelled = true;
                        fm.dropTarget = "";
                        ...
                        // si el grab falla, el drag sale igualmente sin icono.
                        var ok = dragGhost.grabToImage(function(result) {
                            xdndProxy.Drag.imageSource = result.url;
                            selArea.beginPlatformDrag();
                        });
                        if (!ok) beginPlatformDrag();
                    }
                    function beginPlatformDrag() {
                        dragging = false;                   // esconde el ghost interno
                        xdndProxy.Drag.startDrag(Qt.CopyAction);
                    }
                    ...
                    function gestureMove(lx, ly, ctrlHeld) {
                        ...
                        if (marquee) applyMarquee(lx, ly);
                            ...
                            if (wp.x < 0 || wp.y < 0 || wp.x >= fm.width || wp.y >= fm.height) {
                                startExternalDrag();
                                return;
                            }
                    ...
                    function onCursorPoll(gxg, gyg) {
                        // El poll SOLO alimenta el gesto cuando el compositor retiene
                        // el motion (VM/puntero absoluto). Con motion real entregado,
                        // el poll sobra y —con varias instancias del FM con el mismo
                        // título— podría operar con el origen de OTRA ventana.
                        if (sawMotion || !leftHeld || cancelled || !gOffValid) return;
                ...
                Item {
                    ...
                    Drag.onDragFinished: selArea.cancelDrag()
                }
