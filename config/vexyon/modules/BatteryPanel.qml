...
//  de ilyamiro se conserva SOLO la gestión real de energía: cajas de uptime
//  HR:MIN con dos puntos pulsantes, anillo de batería Canvas con halo y
...
AnchoredPanel {
    ...
    content: Component {
        Item {
            ...
            Process {
                id: upPoller
                command: ["bash", "-c", "awk '{print int($1/3600)\":\"int(($1%3600)/60)}' /proc/uptime"]
                running: true
                stdout: StdioCollector {
                    onStreamFinished: {
                        var p = this.text.trim().split(":");
                        body.upHours = parseInt(p[0]) || 0;
                        body.upMins = parseInt(p[1]) || 0;
                    }
                }
            }
            Timer { interval: 30000; running: bp.shown; repeat: true; onTriggered: upPoller.running = true }
                ...
                Row {
                    ...
                    Rectangle {
                        ...
                        Column {
                            anchors.centerIn: parent
                            Text {
                                text: body.upHours.toString().padStart(2, "0")
                    ...
                    Rectangle {
                        ...
                        Column {
                            anchors.centerIn: parent
                            Text {
                                text: body.upMins.toString().padStart(2, "0")
                                font.pixelSize: 17; font.family: Theme.fontFamily; font.weight: Font.Black
