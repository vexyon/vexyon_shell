...
-- con la elección real del usuario en cuanto el bridge corre una vez). SIEMPRE
-- se fija un tema real: sin XCURSOR_THEME Hyprland usa su cursor embebido, que
-- tiene la forma de su logo. El defecto coincide con el seed de shell.json
-- ("pretty" -> Breeze_Light, de breeze-cursors).
...
-- Cursor elegido por el usuario, GENERADO por el bridge en cada cambio. Llega
-- DESPUÉS del hl.env de arriba para pisar el defecto. pcall + ruta absoluta:
-- en una instalación recién hecha puede que aún no exista, y eso no debe
-- tumbar la pantalla de login.
pcall(dofile, "/etc/greetd/vexyon-greeter/cursor.lua")
